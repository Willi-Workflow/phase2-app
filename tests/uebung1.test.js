...
test("Erfüllung Mission 1: Abschüsse je Minute am Bestwert gemessen, Buchstaben fließen ein", () => {
  ...
  // abgeschossen wurde; seit 07.09.2026 sind 12 je Minute die vollen 100
  // (Willis Auftrag: schwerer, schrittweise von 5). Mit Letter-Task 70/30,
  // jeder Fehlalarm kostet 15 Punkte.
  assert.equal(TREFFER_BESTWERT, 12);
  assert.equal(trefferErfuellung(36, 3), 100);
  assert.equal(trefferErfuellung(99, 3), 100);                // Deckel
  assert.ok(Math.abs(trefferErfuellung(12, 2) - 50) < 1e-9);
  assert.equal(trefferErfuellung(0, 3), 0);
  assert.equal(trefferErfuellung(12, 0), 0);                  // ohne Dauer keine Wertung
